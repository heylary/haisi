...
HI_S32 CarDetectInit()
{
    return Yolo2FdLoad(&g_carModel);
}
...
HI_S32 CarDetectExit()
{
    return Yolo2FdUnload(g_carModel);
}

static HI_S32 CarDetect(uintptr_t model, IVE_IMAGE_S *srcYuv, DetectObjInfo boxs[])
{
...
HI_S32 CarDetectCal(IVE_IMAGE_S *srcYuv, DetectObjInfo resArr[])
{
    int ret = CarDetect(g_carModel, srcYuv, resArr);
    return ret;
}
